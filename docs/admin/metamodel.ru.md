...
Оценка **качества данных** карточки — это взвешенная мера её полноты. Каждый влияющий фактор — каждое поле и четыре встроенных фактора — настраивается в одном месте: на вкладке **Качество данных** редактора типа карточки, открываемой значком качества вверху (рядом со значком перевода).
