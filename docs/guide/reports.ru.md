...
### Группировка по роли заинтересованного лица

Если у выбранного типа карточек определены роли заинтересованных лиц и хотя бы одна карточка имеет назначения, в меню **Группировать по** появляется пункт **Роль заинтересованного лица**. Каждая плитка соответствует одной роли (например, *Application Owner*, *Business Owner*), а карточки внутри подписаны именем человека, который выполняет эту роль. Карточка с двумя разными ролями попадает в обе плитки.

В панели инструментов появляется отдельный раздел фильтров **Заинтересованные стороны** с тремя элементами управления:

- **Заинтересованное лицо** — ограничивает вид карточками, где один из выбранных людей выполняет *любую* роль. Отвечает на вопрос «что принадлежит Алисе?».
- **Роль** — ограничивает вид конкретными типами ролей. Отвечает на вопрос «у каких приложений нет Business Owner?».
- **Включая неназначенные** — при группировке по роли заинтересованного лица включите, чтобы добавить отдельную плитку *Неназначенные* с карточками без заинтересованных сторон. По умолчанию выключено, так как у большинства типов карточек много неназначенных элементов.

Эти три фильтра работают и вне группировки по роли — например, сгруппируйте по жизненному циклу и отфильтруйте по конкретному человеку, чтобы увидеть его карточки по фазам жизненного цикла.
