...
当所选卡片类型已定义干系人角色并且至少有一张卡片有干系人分配时，**分组依据** 下拉框会新增 **干系人角色** 选项。每个图块对应一个角色（例如「Application Owner」「Business Owner」），图块内的卡片会显示担任该角色的人员姓名。同一张卡片若同时承担两种不同的角色，会在两个图块中各出现一次。

工具栏中会出现一个专门的 **干系人** 筛选区域，提供三个控件：

- **干系人** —— 仅显示由所选人员担任任意角色的卡片。用于回答「Alice 拥有哪些资产？」
- **角色** —— 仅显示指定角色类型的卡片。用于回答「哪些应用没有 Business Owner？」
- **包含未分配** —— 在按干系人角色分组时打开，可额外显示一个 *未分配* 图块，列出没有任何干系人的卡片。默认关闭，因为多数卡片类型存在大量未分配项。

这三个筛选项在其他分组模式下同样有效——例如按生命周期分组并按某个人筛选，即可查看该人员负责的卡片在各生命周期阶段的分布。
