...
### Fixed
- **MCP HTTP transport now uses Streamable HTTP, served at the right path.** The server was wired to `mcp.sse_app()`, the older two-endpoint SSE transport (`GET /sse` stream + `POST /messages/?session_id=…`). Modern MCP clients (Claude Desktop's custom connector, the MCP Inspector) speak Streamable HTTP — they POST JSON-RPC directly to the protocol endpoint URL with an optional SSE upgrade — so the connector handshake silently failed against the SSE app. Two changes: switch to `mcp.streamable_http_app()`, and stop mounting it inside an outer Starlette app at `/mcp` (the mount pushed the protocol route to `/mcp/mcp` on the upstream and triggered a 307 redirect for clients hitting `/mcp` without a trailing slash). The streamable app is now the top-level app with the OAuth + well-known + health routes attached directly to it, so nginx's `/mcp/` strip lines up with the upstream's `/mcp`, `/.well-known/*`, and `/oauth/*` routes.
